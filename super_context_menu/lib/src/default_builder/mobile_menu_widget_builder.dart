    ...
    switch (brightness) {
      case Brightness.light:
        return DefaultMobileMenuTheme(
          ...
          backgroundTintColor: (hasBlur) =>
              hasBlur ? const Color(0xF3333333).withOpacity(0.3) : const Color(0xFF333333).withOpacity(0.5),
...
class DefaultMobileMenuWidgetBuilder extends MobileMenuWidgetBuilder {
  ...
  Widget buildInactiveMenuVeil(
    ...
    return AnimatedContainer(
        duration: const Duration(milliseconds: 200), color: theme.inactiveMenuVeilColor(menuInfo.depth));
  }
...
class _MenuItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (element is Menu) {
      prefix = Builder(builder: (context) {
        return _AnimatedChevron(
          ...
          chevronSize: (element as Menu).chevronSize,
        );
      });
    } else {
      prefix = null;
    ...
    if (element is DeferredMenuElement) {
      ...
      return Container(
        height: (element as MenuSeparator).height,
        margin: const EdgeInsets.symmetric(vertical: 1),
        color: (element as MenuSeparator).color ?? theme.separatorColor,
      );
    } else {
...
class _AnimatedChevron extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final offset = !isExpanded ? const Offset(0.0, -0.05) : const Offset(0.0, -0.05);
